...
export class ProjectDetails implements OnInit {
  ...
  ngOnInit() {
    if (history.state?.project) {
      console.log('Project received from history state:', history.state.project);
      this.project = history.state.project as Project;
      //console.log('Project received from history state:', this.project);
    ...
    this.environments$ = this.project$.pipe(map((project) => this.buildEnvironments(project)));

    this.deployments$ = this.project$.pipe(map((project) => this.buildDeployments(project)));

    combineLatest([this.environments$, this.deployments$])
      ...
      actions: [
        {
          label: 'View',
          icon: Eye,
          color: 'bg-white text-gray-600 border border-black/5 select-none',
          action: 'view',
        },
  ...
  handleEnvironmentAction(event: { action: string; row: any }) {
    if (event.action === 'view') {
      ...
      console.log(
        'Navigating to environment detail page for project ID:',
        this.project?.id,
        'and environment name:',
        event.row.environmentName,
      );
      //this.project?.id,
      //(event.row.environmentName,
      this.router.navigate([
        '/projects/environment-detail',
        this.project?.id,
        event.row.environmentName,
      ]);
    }
